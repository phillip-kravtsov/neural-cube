...
#full_data = np.genfromtxt("2bindata.txt")
#train_data, test_data = np.split(full_data, [(85 * full_data.shape[0])/100])
#val_data, train_data = np.split(train_data, [(1 * train_data.shape[0])/100])
graph = tf.Graph()
...
#n_fc4 = 100
  ...
  def net(x, w, b):
    ...
    #fc4 = tf.nn.relu(tf.matmul(fc3, weights['w4']) + b['b4'])
  ...
  with tf.variable_scope("p1net", initializer=tf.random_normal_initializer(0.1,0.1)):
    ...
    weights = {
      ...
      #'w4' : tf.Variable(tf.random_normal([n_fc3, n_fc4])),
  ...
  #'b4' : tf.Variable(tf.random_normal([n_fc4])),
